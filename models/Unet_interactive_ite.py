...
class unet2D(nn.Module):
    def __init__(self, layers, s_num_classes=1):
        ...
        self.controller = nn.Conv2d(256+2, 162, kernel_size=1, stride=1, padding=0)   #### change the channel
        # self.controller = nn.Conv2d(256+6, 162, kernel_size=1, stride=1, padding=0)
    ...
    #     # print(feature.shape, coord_feature.shape)
    #     result = torch.bmm(feature.transpose(1, 2), feature)
    #     scale_factor = torch.sqrt(torch.tensor(h*w, dtype=torch.float32))
    #     result = self.softmax(result / scale_factor)
    #     result = torch.bmm(coord_feature, result)
    #     result = result.view(b, c, h, w) 
    #     return result

    def forward(self, input, point=None, pred=None):
